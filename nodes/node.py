...
def qubit_samples(qubit: str, transition: str = '01', sweep_range = None) -> np.ndarray:
    ...
    if sweep_range is None: sweep_range = 50e6
    key_backup = f"transmons:{qubit}", "freq_" + transition + "_backup"
    key = f"transmons:{qubit}", "freq_" + transition
    redis_value = cxn.hget(*key)
    redis_value_backup = cxn.hget(*key_backup)
    VNA_value = VNA_qubit_frequencies[qubit] if transition == '01' else VNA_f12_frequencies[qubit] if transition == '12' else ValueError(f"Invalid transition {transition}")
    if redis_value is not None and redis_value != 'nan':
...
class Qubit_12_Spectroscopy_Multidim_Node(Base_Node):
    ...
    def samplespace(self):
        cluster_samplespace = {
            'spec_pulse_amplitudes': {
                 qubit: np.linspace(30e-4, 30e-4, 1) for qubit in self.all_qubits
            },
            'spec_frequencies': {
...
class Reset_Chevron_Node(Base_Node):
    ...
    def samplespace(self):
        ...
        cluster_samplespace = {
            ...
            # For DC reset
            # 'cz_pulse_durations': {
            #     qubit: 1e-9+np.arange(0e-9, 800e-9,20e-9) for qubit in self.coupled_qubits
            # },
            # 'cz_pulse_amplitudes': {
            #     qubit: np.linspace(0.02, 0.07 , 20) for qubit in self.coupled_qubits
            # },

            'cz_pulse_durations': {
                qubit: 4e-9+np.arange(0e-9, 1200e-9,50e-9) for qubit in self.coupled_qubits
            },
            'cz_pulse_amplitudes': {
                qubit: np.linspace(-0.05, -0.125, 21) for qubit in self.coupled_qubits
            },
            # 'cz_pulse_amplitudes': {
            #     qubit: np.linspace(0.05, 0.15, 21) for qubit in self.coupled_qubits
            # },
...
class CZ_Chevron_Node(Base_Node):
    def __init__(self, name: str, all_qubits: list[str], ** node_dictionary):
        self.couplers = node_dictionary.pop('couplers')
    ...
    def transition_frequency(self):
        ...
        ac_freq_list = [np.abs(q1_f01 + q2_f01 - (q1_f01 + q1_f12)),np.abs(q1_f01 + q2_f01 - (q2_f01 + q2_f12))]
        ac_freq = np.min(ac_freq_list)
    ...
    def samplespace(self):
        ...
        cluster_samplespace = {
            ...
            # For CZ gate
            'cz_pulse_durations': {
                qubit: 12e-9+np.arange(0e-9, 32*20e-9,32e-9) for qubit in self.coupled_qubits
            },
            'cz_pulse_frequencies_sweep': {
            ...
            # 'cz_pulse_durations': {
            #     qubit: 12e-9+np.arange(0e-9, 32*20e-9,20e-9) for qubit in self.coupled_qubits
            # },
            # 'cz_pulse_frequencies_sweep': {
            #     qubit: np.linspace(-24e6,0e6, 21) + self.ac_freq for qubit in self.coupled_qubits
            # },
