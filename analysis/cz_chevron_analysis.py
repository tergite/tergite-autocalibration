...
class ChevronModel(lmfit.model.Model):
    ...
    def guess(self, data, **kws) -> lmfit.parameter.Parameters:
        ...
        self.set_param_hint("oscillation_frequency", value=freq_guess, min=0)
...
class CZChevronAnalysis():
    ...
    def run_fitting(self):
        ...
        if int(self.qubit[1:]) % 2 == 0:
            ...
            optimal_phase = fit_results[opt_id].result.params['phase'].value + np.pi
            qubit_type = 'target'
        else:
        ...
        print(f'{ optimal_phase = }')
        self.opt_cz = cz_duration * ( np.pi - optimal_phase) / 2 / np.pi
        print(f'{ self.opt_cz = }')
    ...
    def plotter(self,axis):
        ...
        axis.hlines(self.opt_freq,self.amp[0],self.amp[-1],label = 'Frequency Detuning = {:.2f} MHz'.format(self.opt_freq/1e6),colors='k',linestyles='--',linewidth=1.5)
