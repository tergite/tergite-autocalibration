...
def precompile(node, bin_mode:str=None, repetitions:int=None):
    ...
    if node.name in ['cz_chevron','cz_calibration','cz_calibration_ssro','cz_dynamic_phase','reset_chevron','reset_calibration_ssro']:
        coupler = node.coupler
        node_class = node.measurement_obj(transmons, coupler, node.qubit_state)
    if node.name in ['ro_amplitude_optimization_gef','cz_calibration_ssro','reset_calibration_ssro']:
