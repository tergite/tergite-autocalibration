...
### Calibration configuration (.toml):
This file is solely configuring the calibration process itself i.e. everything that is related to running the calibration, its measurements and analyses.
In the `[general]` section there are some settings such as the target node, the qubits and the couplers to calibrate.
