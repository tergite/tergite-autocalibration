...
from tergite_calibration.config.coupler_config import qubit_types
from tergite_calibration.config.settings import REDIS_CONNECTION
from tergite_calibration.lib.measurement_base import Measurement
from tergite_calibration.utils.extended_transmon_element import Measure_RO_Opt, Rxy_12
    ...
    def schedule_function(
        ...
        for cz_index, control_on in enumerate(control_on_values):
            for ramsey_index, ramsey_phase in enumerate(ramsey_phases_values):
                ...
                for this_qubit in all_qubits:
                    if qubit_types[this_qubit] == 'Target':
                        x90_end = schedule.add(Rxy(theta=90, phi=ramsey_phase, qubit=this_qubit), ref_op=buffer,
                                               ref_pt='end')
