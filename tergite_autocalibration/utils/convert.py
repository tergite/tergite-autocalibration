...
def store_manual_parameters(qubit_id):
    ...
    for manual_parameter_key, manual_parameter_value in manual_param_map.items():
        structured_redis_storage(
            manual_parameter_key, qubit_id.strip("q"), manual_parameter_value
        )


def manual_checks(
    parameter_name_: str, parameter_value_: Any, overwrite_default: bool = False
) -> Any:
...
def convert_all_redis_values(component_ids: List[str] = None):
    ...
    def read_redis_value_calibration_format(
        component_prefix: str, component_id: str, parameter_key: str
    ):
        # TODO: add some error handling
        value_ = REDIS_CONNECTION.hget(
            f"{component_prefix}:{component_id}", parameter_key
        )
        return value_
    ...
    for qubit in qubits:
        for parameter_name in param_map.keys():
            parameter_value = read_redis_value_calibration_format(
                "transmons", qubit, parameter_name
            )
            parameter_value = manual_checks(
                parameter_name, parameter_value, overwrite_default=True
            )
            if parameter_value is not None:
                structured_redis_storage(
                    parameter_name, qubit.strip("q"), parameter_value
                )
