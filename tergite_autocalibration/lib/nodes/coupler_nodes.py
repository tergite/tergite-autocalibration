...
from tergite_autocalibration.lib.analysis.cz_calibration_analysis import (
...
from tergite_autocalibration.lib.analysis.cz_chevron_analysis import (
    CZChevronAnalysisReset,
    CZChevronAmplitudeAnalysis,
)
from tergite_autocalibration.lib.analysis.cz_firstStep_analysis import (
    CZFirtStepAnalysis,
)
from tergite_autocalibration.lib.analysis.randomized_benchmarking_analysis import (
    RandomizedBenchmarkingAnalysis,
)
from tergite_autocalibration.lib.analysis.reset_calibration_analysis import (
...
from tergite_autocalibration.lib.calibration_schedules.cz_chevron_reversed import (
    CZ_chevron,
)
from tergite_autocalibration.lib.calibration_schedules.cz_chevron_reversed import (
...
class Coupler_Spectroscopy_Node(BaseNode):
    ...
    def __init__(self, name: str, all_qubits: list[str], **node_dictionary):
        ...
        self.schedule_samplespace = {
            "spec_frequencies": {
                self.measurement_qubit: qubit_samples(self.measurement_qubit)
            }
        }

        self.external_samplespace = {
            "dc_currents": {self.coupler: np.arange(-2.5e-3, 2.5e-3, 250e-6)},
        }
        # self.validate()
        self.node_externals = (
            np.round(np.append(np.arange(0.1, 3.1, 0.1), np.arange(-3, 0.1, 0.1)), 1)
            * 1e-3
        )
        ...
        self.coupler = self.couplers[
            0
        ]  # we will generalize for multiple couplers later
...
class Coupler_Resonator_Spectroscopy_Node(BaseNode):
    ...
    def samplespace(self):
        ...
        cluster_samplespace = {"ro_frequencies": {qubit: resonator_samples(qubit)}}
        return cluster_samplespace
