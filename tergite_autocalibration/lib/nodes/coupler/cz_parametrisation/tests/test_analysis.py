...
from tergite_autocalibration.lib.nodes.coupler.cz_parametrisation.utils.NoValidCombinationException import NoValidCombinationException
...
def setup_data():
    ...
    q14Ana = CZ_Parametrisation_Frequency_vs_Amplitude_Q1_Analysis(d14, freqs_bad, amps_bad)
    q14Res = q14Ana.run_fitting()
    q15Ana = CZ_Parametrisation_Frequency_vs_Amplitude_Q2_Analysis(d15, freqs_bad, amps_bad)
    q15Res = q15Ana.run_fitting()
...
def test_PickLowestCurrentWithoutBest(setup_data):
    ...
    assert a.opt_index == 1 #I removed the first, so the good point is now the last which is in position 1
