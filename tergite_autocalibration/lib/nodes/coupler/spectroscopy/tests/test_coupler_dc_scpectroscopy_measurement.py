...
def test_measurement_01_type():
    ...
    node = CouplerDCSpectroscopyNode(CONFIG.run.couplers)
...
def test_dummy_generation():
    ...
    number_of_qubit_frequencies = len(
        node.schedule_samplespace["qubit_frequencies"][first_qubit]
    )
    number_of_resonator_frequencies = len(
        node.schedule_samplespace["resonator_frequencies"][first_qubit]
    )
    measured_qubits = [
    ...
    assert (
        ...
        == number_of_qubit_frequencies + number_of_resonator_frequencies
    )
