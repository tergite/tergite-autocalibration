...
from tergite_autocalibration.lib.nodes.coupler.cz_calibration.node import CZCalibrationSSRONode
...
class TQGRandomizedBenchmarkingNode(ParametrizedSweepNode):
...
class TQGRandomizedBenchmarkingInterleavedNode(ParametrizedSweepNode):
    measurement_obj = TQG_Randomized_Benchmarking
    analysis_obj = RandomizedBenchmarkingNodeAnalysis
...
class TQGRandomizedBenchmarkingSSRONode(ParametrizedSweepNode):
    def __init__(self, name: str, all_qubits: list[str], couplers: list[str], **schedule_keywords):
        ...
        self.backup = False
        self.redis_field = ['tqg_fidelity']
        self.measurement_obj = TQG_Randomized_Benchmarking_SSRO
        self.analysis_obj = RandomizedBenchmarkingSSROAnalysis
        self.qubit_state = 2
        # TODO change it a dictionary like samplespace

        self.external_samplespace = {
            "seeds": {coupler: np.arange(RB_REPEATS, dtype=np.int32) for coupler in self.couplers}
        }

        self.initial_schedule_samplespace = {
            'number_of_cliffords': {
                coupler: np.append(np.array([0, 1, 2, 3, 4, 8, 10, 16, 22, 32, 64, 128]), [0, 1, 2]) for coupler in self.couplers
    ...
    @property
    def dimensions(self):
        return [len(self.initial_schedule_samplespace['number_of_cliffords'][self.couplers[0]]), 1]
...
class CZRBOptimizeSSRONode(ParametrizedSweepNode):
    ...
    def __init__(
        ...
        self.study = optuna.create_study(sampler=sampler, study_name = self.name+'_'+self.time_id)
        self.average = False
        self.trails = 70
        self.opt_params = ["cz_duration", "cz_frequency"]
        self.full_params = ["cz_duration", "cz_frequency", "cz_amplitude"]
        self.all_results_list = []
        self.original_cz_param = self.redis_connection.hgetall(f"couplers:{self.coupler}")
    ...
    def objective(self, trial):
        ...
        for param in self.full_params:
            if param in self.opt_params:
                if param == "cz_duration":
                    values = (
                        np.array(
                            [trial.suggest_float("cz_duration", -25, 25, step=1)]
                        )*1e-9
                    )
                    ...
                    values = np.array(
                        [
                            trial.suggest_float(
                                "cz_amplitude", -0.1, 0.1, step=0.001
                            )
                        ]
                    )
                    param_dict[param] = dict(zip(self.couplers, values))
        ...
        if param_dict['cz_frequency'] is not None:
            cz_param['cz_pulse_frequency'] = float(cz_param['cz_pulse_frequency']) + param_dict['cz_frequency'][self.coupler]
        if param_dict['cz_duration'] is not None:
            cz_param['cz_pulse_duration'] = float(cz_param['cz_pulse_duration']) + param_dict['cz_duration'][self.coupler]
        if param_dict['cz_amplitude'] is not None:
            cz_param['cz_pulse_amplitude'] = float(cz_param['cz_pulse_amplitude']) + param_dict['cz_amplitude'][self.coupler]    
        ...
        node_dynamic_phase = CZDynamicPhaseNode("cz_dynamic_phase", self.all_qubits, self.couplers)
        
        dynamic_phase = node_dynamic_phase.calibrate(self.data_path, self.lab_ic, self.cluster_status)

        node_dynamic_phase_swap = CZDynamicPhaseSwapNode("cz_dynamic_phase_swap", self.all_qubits, self.couplers)
        
        dynamic_phase_swap = node_dynamic_phase_swap.calibrate(self.data_path, self.lab_ic, self.cluster_status)
        ...
        if qubit_types[self.coupled_qubits[0]] == self.qubit_type_list[0]:
            cz_param['cz_dynamic_target'] = -1* dynamic_phase[self.coupled_qubits[1] + coupler_append]['cz_dynamic_target']
            cz_param['cz_dynamic_control'] = -1 * dynamic_phase_swap[self.coupled_qubits[0] + coupler_append]['cz_dynamic_control']
        else:
            cz_param['cz_dynamic_target'] = -1 * dynamic_phase[self.coupled_qubits[0] + coupler_append]['cz_dynamic_target']
            cz_param['cz_dynamic_control'] = -1 *dynamic_phase_swap[self.coupled_qubits[1] + coupler_append]['cz_dynamic_control']
            
        for key, value in cz_param.items():
            self.redis_connection.hset(f"couplers:{self.coupler}", key,value)

        rb_node = TQGRandomizedBenchmarkingInterleavedSSRONode("tqg_randomized_benchmarking_interleaved_ssro", 
                                                                    self.all_qubits, self.couplers)
        
        tqg_rb_results = rb_node.calibrate(self.data_path, self.lab_ic, self.cluster_status)


        all_results = [tqg_rb_results[qubit + coupler_append] for qubit in self.coupled_qubits]

        print(f"Results: {all_results}")

        all_costs = [ np.sqrt(1-res['tqg_fidelity_interleaved'])**2 for res in all_results]

        original_cz_param = cz_param
        if param_dict['cz_frequency'] is not None:
            original_cz_param['cz_pulse_frequency'] = float(cz_param['cz_pulse_frequency']) - param_dict['cz_frequency'][self.coupler]
        if param_dict['cz_duration'] is not None:
            original_cz_param['cz_pulse_duration'] = float(cz_param['cz_pulse_duration']) - param_dict['cz_duration'][self.coupler]
        if param_dict['cz_amplitude'] is not None:
            original_cz_param['cz_pulse_amplitude'] = float(cz_param['cz_pulse_amplitude']) - param_dict['cz_amplitude'][self.coupler]     
        ...
        self.all_results_list.append({'path':str(self.data_path),'trial':trial.number, 'cost': sum(all_costs), 'param_dict':param_dict,'results':tqg_rb_results,\
                                       'original_cz_parameters': original_cz_param, 'current cz parameters:': cz_param})
        
        with open(self.log_path+self.time_id+'_cz_optimization_RB_all_results.py', 'w') as f:
    ...
    def validate(self, name = 'validate'):
        ...
        print('cz_param is:', cz_param)
        if param_dict['cz_frequency'] is not None:
            cz_param['cz_pulse_frequency'] = float(cz_param['cz_pulse_frequency']) + param_dict['cz_frequency'][self.coupler]*1e6
        if param_dict['cz_duration'] is not None:
            cz_param['cz_pulse_duration'] = float(cz_param['cz_pulse_duration']) + param_dict['cz_duration'][self.coupler]*1e-9
        if param_dict['cz_amplitude'] is not None:
            cz_param['cz_pulse_amplitude'] = float(cz_param['cz_pulse_amplitude']) + param_dict['cz_amplitude'][self.coupler]     
        ...
        node_dynamic_phase = CZDynamicPhaseNode("cz_dynamic_phase", self.all_qubits, self.couplers, **self.schedule_keywords)
        ...
        cz_node = CZCalibrationSSRONode("cz_calibration_ssro", self.all_qubits, self.couplers)
        
        cz_results = cz_node.calibrate(self.data_path, self.lab_ic, self.cluster_status)

        cz_and_rb_results = tqg_rb_results | cz_results

        self.all_results_list.append({'path':str(self.data_path),'trial': name,'param_dict':param_dict,'results':cz_and_rb_results, 'cz parameters': cz_param})
        with open(self.log_path+self.time_id+'_cz_optimization_all_results.py', 'w') as f:
            f.write(f"all_results = {self.all_results_list}\n\n")
    ...
    def calibrate(self, data_path: Path, lab_ic, cluster_status):
        ...
        if self.average:
            ...
            with open(self.log_path+self.time_id+'_cz_optimization_best.py', 'w') as f:
