...
class CheckCliffordsAnalysis(BaseAnalysis):
    ...
    def run_fitting(self):
        ...
        self.normalized_magnitudes = (self.magnitudes - self.magnitudes[0]) / (
    ...
    def plotter(self, ax):
        ...
        ax.hlines(
            y=self.magnitudes[0],
            xmin=self.independents[0],
            xmax=self.independents[-1],
            color="g",
            linestyle="--",
        )  # Plots |0⟩
        ax.hlines(
        ...
        ax.set_xlabel(
            "Clifford gate index"
        )  # from utils/clifford_elements_decomposition.py
        ax.grid()
