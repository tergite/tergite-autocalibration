...
class CheckCliffordsAnalysis(BaseAnalysis):
    ...
    def plotter(self, ax):
        # unnormalized plot:
        ax.plot(self.independents[:-2], self.theoretical_magnitudes, 'ro', lw=3.0,
                label=f"Theoretical clifford magnitudes")
        ax.plot(self.independents[:-2], self.normalized_magnitudes[2:], 'bo', ms=3.0,
                label=f"Measured clifford magnitudes")
        ax.hlines(y=self.magnitudes[0], xmin=self.independents[0], xmax=self.independents[-1], color='g',
                  linestyle='--')  # Plots |0⟩
        ax.hlines(y=self.magnitudes[1], xmin=self.independents[0], xmax=self.independents[-1], color='g',
                  linestyle='--')  # Plots |1⟩
