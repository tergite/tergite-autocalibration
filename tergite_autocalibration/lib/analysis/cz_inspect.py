...
def inspect(data, q0, q1, savefig=None, fig_suffix="png"):
    ...
    magnitudes = np.transpose(
        (magnitudes - np.min(magnitudes)) / (np.max(magnitudes) - np.min(magnitudes))
    )
    ...
    for i, prob in enumerate(magnitudes):
        ...
        def fitfunc(p):
            return (
                p[0] * np.exp(-p[4] * times) * np.cos(2 * np.pi / p[1] * (times - p[2]))
                + p[3]
            )
    ...
    if len(cs) < 5:
        ...
        if p[2] > freq[-1] or p[2] < freq[0] or p[0] < 0 or p[1] < 0:
            ...
            axes[2].vlines(
                f_opt, np.min(cs), np.max(cs), "g", linestyle="--", linewidth=1.5
            )
            axes[2].set_title(
                f"Result for {data.tuid}. The optimal frequency is {f_opt}."
            )
