...
if netstat -tuln | grep -q ":$REDIS_USER_PORT"; then
...
sudo install -o redis -g redis -d "/var/lib/redis_$REDIS_USER_VARIABLE"
...
sudo cp -p "$TEMP_CONF_FILE" "/etc/redis/redis_$REDIS_USER_VARIABLE.conf"
...
sed "s/REDIS_USER_VARIABLE/$REDIS_USER_VARIABLE/g" redis-server-template.conf > "$TEMP_SERVICE_FILE"
# Copy the service file
sudo cp "$TEMP_SERVICE_FILE" /lib/systemd/system/redis-server-"$REDIS_USER_VARIABLE".service
# Cleanup the temporary file
rm "$TEMP_SERVICE_FILE"

# Try to enable and start the service
sudo systemctl enable redis-server-"$REDIS_USER_VARIABLE".service
sudo systemctl start redis-server-"$REDIS_USER_VARIABLE".service
