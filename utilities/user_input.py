...
def experiment_parameters(node:str, qubits:List[str], dummy:bool=False) -> dict:
    ...
    sweep_parameters = {
        ...
        'two_tone_multidim': {
            ...
            'spec_pulse_amplitudes': {qubit : np.linspace(2e-5, 1e-3, 5) for qubit in qubits}
        },
        ...
        'ramsey_correction_12': {
            'ramsey_delays': { qubit : np.arange(4e-9, 1*2048e-9, 8*8e-9) for qubit in qubits },
            'artificial_detunings': { qubit : np.arange(-2.1, 2.1, 0.8)*1e6 for qubit in qubits },
        },
        ...
        'coupler_spectroscopy': {
            'spec_frequencies': {'q25': qubit_samples('q25')},
            'dc_currents': {'q25': np.arange(-3e-3, 3e-3, 100e-6)}
        },
    }
